...
def file_overlap(url1, url2):
	...
	with open('fileOverlap1.txt', 'w') as open_file:
		open_file.write(str(soup1))

	r2 = requests.get(url2)
	soup2 = str(BeautifulSoup(r2.text, 'html.parser'))

	with open('fileOverlap2.txt', 'w') as open_file:
		open_file.write(str(soup2))

	list1 = []
	listOverlap = []

	with open('fileOverlap1.txt', 'r') as open_file:	
		for line in open_file:
			number = line.strip()
			list1.append(int(number))

		with open('fileOverlap2.txt', 'r') as open_file:	
			for line in open_file:
